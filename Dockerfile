...
RUN apk add --no-cache ca-certificates tzdata curl && \
    curl -sSL -o /usr/local/bin/kubectl "https://dl.k8s.io/release/v1.31.4/bin/linux/amd64/kubectl" && \
    chmod +x /usr/local/bin/kubectl && \
    curl -sSL -o helm.tar.gz "https://get.helm.sh/helm-v3.16.4-linux-amd64.tar.gz" && \
    mkdir -p helm && \
